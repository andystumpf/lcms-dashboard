// Filter and aggregation math for tests, the API, and the browser
// (js/lcms-math-boot.mjs assigns this module to window.LCMSMath).

import { duplicatePlaceKeys } from './church-search.mjs';
import { evaluateStoryMath, LEAGUE_TABLE_SUBTITLE } from './story-math.mjs';

export { LEAGUE_TABLE_SUBTITLE };
...
export function topN(churches, field, n = 50, dir = 'desc', { membersOnly = false } = {}) {
  const list = membersOnly ? churches.filter(isMemberCongregation) : churches;
  return [...list]
...
export function isMemberCongregation(c) {
  return (c.status || '').trim() === 'Member Congregation';
}

export function churchHistoryEnd(c) {
  const years = c?.history?.years;
  return years?.length ? years[years.length - 1] : null;
}

export function churchHeadlineYear(c) {
  return c?.lastStatYear || c?.reportYear || churchHistoryEnd(c) || null;
}

export function hasHeadlineHistoryYearGap(c) {
  const head = churchHeadlineYear(c);
  const end = churchHistoryEnd(c);
  return head != null && end != null && Number(head) !== Number(end);
}

/** Note when compare-table headlines and trend charts cover different years. */
export function compareYearMismatchNote(churches) {
  const list = churches || [];
...
export function describeSnapshot(LCMS) {
  ...
  const givingHistoryMillions = years.length
    ? (LCMS.yearly.totalGivingMillions?.[years.length - 1] ?? null)
    : null;
  let givingSum = 0;
  let hasGiving = false;
  for (const c of churches) {
    if (c.giving) { givingSum += c.giving; hasGiving = true; }
  }
  return {
    ...
    duplicateNameGroups: duplicatePlaceKeys(churches).size,
    givingHistoryMillions,
    givingHeadlineMillions: hasGiving ? +(givingSum / 1e6).toFixed(1) : null
  };
}
...
export function snapshotLegendText(snap) {
  ...
  const official = snap.officialCongregations != null
    ...
    : '—';
  let text = `Headlines are each church’s latest reported year (mostly ${head}). `
  ...
  if (
    snap.givingHeadlineMillions != null
    && snap.givingHistoryMillions != null
    && snap.givingHeadlineMillions !== snap.givingHistoryMillions
  ) {
    text += ` Congregation PDF contributions ($${snap.givingHeadlineMillions.toLocaleString()}M) are not the ${snap.historyEnd} synod giving series ($${snap.givingHistoryMillions.toLocaleString()}M).`;
  }
  return text;
}

export function describeHealth(LCMS) {
  const snap = describeSnapshot(LCMS);
  const years = LCMS.yearly?.years || [];
  const historyEnd = snap.historyEnd;
  let headlineHistoryMismatch = 0;
  for (const c of LCMS.churches || []) {
    if (c.baptized == null || !c.history?.years?.length) continue;
    const i = c.history.years.indexOf(historyEnd);
    if (i < 0) continue;
    if (c.history.baptized[i] != null && c.baptized !== c.history.baptized[i]) {
      headlineHistoryMismatch++;
    }
  }
  return {
    ok: true,
    churches: snap.churches,
    districts: LCMS.districts?.length ?? 0,
    officialCongregations: snap.officialCongregations,
    withHistory: snap.withHistory,
    historyStart: snap.historyStart,
    historyEnd,
    historyYears: years.length,
    headlineYear: snap.headlineYear,
    fetchedAt: snap.fetchedAt,
    headlineHistoryMismatch,
    districtYearly: Object.keys(LCMS.districtYearly || {}).length,
    members: (LCMS.churches || []).filter(isMemberCongregation).length,
    duplicateNameGroups: snap.duplicateNameGroups,
    givingHistoryMillions: snap.givingHistoryMillions,
    givingHeadlineMillions: snap.givingHeadlineMillions,
    storyMathErrors: evaluateStoryMath(LCMS).errors.length
  };
}
