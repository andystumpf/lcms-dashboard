// Type-ahead ranking over congregation records. Shared by tests and documented
// for js/church-search.js (browser copy must stay in sync).
...
export function duplicatePlaceKeys(churches) {
...
export function searchChurches(churches, q, { districts = [], maxResults = MAX_SEARCH_RESULTS } = {}) {
  if (!q || q.length < 2 || !churches?.length) return { hits: [], total: 0 };
  const dash = q.indexOf(' — ');
  if (dash >= 0) q = q.slice(0, dash).trim();
  let tokens = q.toLowerCase().split(/[\s,]+/).filter(Boolean);
  const districtNames = new Set(districts.map(d => (d.name || d || '').toLowerCase()).filter(Boolean));
  ...
  return {
    hits: ranked.slice(0, maxResults).map(([, c]) => c),
    total: ranked.length
  };
}
