...
import { isReadQuery, MAX_SQL_ROWS, truncateRows } from './sql-read-query.mjs';
import {
  isHiddenTable,
...
export async function executeSql(db, query) {
  ...
  try {
    ...
    const raw = results ?? [];
    const { rows, truncated, row_count } = truncateRows(raw, MAX_SQL_ROWS);
    const columns = rows.length
      ? Object.keys(rows[0])
      : (meta?.columns?.map(c => c.name) ?? []);
    const extra = truncated ? ` Results truncated to ${MAX_SQL_ROWS} rows.` : '';
    return {
      success: true,
      type: 'SELECT',
      columns,
      ...
      message: `Query executed successfully. ${row_count} rows returned.${extra}`
    };
  } catch (err) {
